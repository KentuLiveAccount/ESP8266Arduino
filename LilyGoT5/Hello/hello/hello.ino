...
bool FPin9On()
{
  return digitalRead(39) != HIGH; // pulled up gate
}
...
void loop()
{
  ...
  bool fLRClick = false;
  bool fBLEOn = bleMouse.isConnected();
  if (fBLEOn && fMouseOn)
  {
  ...
  if (abs(millisNow - millisLast) > 10000)
  {
    ...
    if (!fMouseOn)
    {
      ...
      display.println(fBLEOn ? "BLE connected" : "BLE not connected");
      display.println(fButtonDown ? "ButtonDown" : "ButtonUp");
      display.println(fMouseOn ? "Mouse on" : "Mouse off");
